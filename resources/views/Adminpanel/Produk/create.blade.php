...
@section('admincontent')
    <div class="container">
        <div class="card">
            <div class="card-body">
                <form action="{{ url('adminpanel/produk') }}" method="post" enctype="multipart/form-data">
                    @csrf
                    <h2> Add Product</h2>
                    <div class="form-group">
                        <label for="exampleFormControlInput1">Name</label>
                        <input type="text" name="name" class="form-control @error('name') is-invalid @enderror" id="exampleFormControlInput1" placeholder="Nama"  value="{{ old('name') }}">
                        @error('name')
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" class="form-control  @error('price') is-invalid @enderror" name="price" id="exampleFormControlInput1" placeholder="Harga"  value="{{ old('price') }}">
                        @error('name')
                        <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" name="stock" class="form-control @error('name') is-invalid @enderror" id="exampleFormControlInput1" placeholder="Stok" value="{{ old('stock') }}">
                        @error('stock')
                        <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                        ...
                        <select class="form-control" id="exampleFormControlSelect1" name="product_category_id">
                            ...
                            @foreach ($category_products as $category_product)
                                <option value="{{ $category_product->id }}">
                                    {{ old('product_category_id') == $category_product->id ? 'selected' : '' }} {{ $category_product->name }}
                                </option>
                            @endforeach
